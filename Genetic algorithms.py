...
def print_table(population, fitness_values, prob_values, expected_output, actual_count):
    print(
        f"{'DNA Seq No':<10} | {'DNA Sequence (bin)':<18} | {'x Value':<7} | "
        f"{'Fitness (DNA match)':<20} | {'% Prob':<7} | {'Expected Count':<15} | {'Actual Count':<12}"
    )
    print("-" * 105)

    for i, (chrom, x, fit, prob, exp, act) in enumerate(
        zip(
            population,
        ...
        print(
            f"{i:<10} | {chrom:<18} | {x:<7} | {fit:<20} | "
            f"{prob * 100:<7.2f} | {exp:<15.2f} | {act:<12}"
        )
...
def selection(population):
    ...
    print(
        f"\nSum of Fitness (DNA matches): {total_fitness}, "
        f"Max Fitness: {max_fitness}, Avg Fitness: {avg_fitness:.2f}"
    )
...
def mating_pool_selection(population, pool_size, bit_pos):
    ...
    print_mating_pool(
        mating_pool, f"Mating Pool before crossover at bit {bit_pos} (DNA sequence bits)"
    )
    ...
    print_mating_pool(
        mating_pool, f"Mating Pool after crossover at bit {bit_pos} (DNA sequence bits)"
    )
...
def main():
    ...
    for _ in range(num_chromosomes):
        while True:
            ...
            if len(chrom) == length_chromosome and set(chrom).issubset({"0", "1"}):
                ...
                print(
                    f"Invalid input. Enter binary string of length {length_chromosome}."
                )
    ...
    while True:
        ...
        while True:
            pool_size = int(
                input(
                    f"\nEnter mating pool size (even number ≤ DNA sequence bit length "
                    f"{length_chromosome} and ≤ population size {len(population)}): "
                )
            )
            if (
                pool_size <= length_chromosome
                and pool_size <= len(population)
                and pool_size > 0
                and pool_size % 2 == 0
            ):
                break
                ...
                print(
                    "Invalid input. Mating pool size must be an even number, > 0, "
                    "and ≤ DNA sequence length and population size."
                )
...
/*output*/
=== DNA Sequence Matching Genetic Algorithm ===
